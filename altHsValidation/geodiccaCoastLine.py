...
class coastLine:
  ...
  def __init__(self, resolution = 'c', llcrnr=None, urcrnr=None):
    ...
    if llcrnr is None:
      ...
      self.basemap = basemap.Basemap(resolution = resolution, llcrnrlon=llcrnr[0], llcrnrlat=llcrnr[1], urcrnrlon=urcrnr[0], urcrnrlat=urcrnr[1])
  ...
  def getDistanceVectorFromCoast(self, lon, lat):
    ...
    def pointInFrontOfCoastSegment(coastpt0, coastpt1):
      result, xyintp[0], xyintp[1] = normalIntersectSegment((lon, lat), coastpt0, coastpt1)
      return result
    ...
    xdist = (distanceDeg[0] * cos(lat * deToRa)) * earthCircle / 360.
    ydist = distanceDeg[1] * earthCircle / 360.
    return (xdist, ydist)
