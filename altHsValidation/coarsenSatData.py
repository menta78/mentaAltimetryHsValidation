...
class satDataManager:
  ...
  def getSatNames(self):
    satNames = [re.sub('(.*)_(.*)(\.npy)', '\\1', f)\
           for f in os.listdir(self.dataDir) if re.match('(.*)_(.*)\.npy', f)]
    return set(satNames)

  def getObsYears(self):
    yrs = [int(re.sub('(.*)_(.*)(\.npy)', '\\2', f))\
           for f in os.listdir(self.dataDir) if re.match('(.*)_(.*)\.npy', f)]
    yrs = np.unique(yrs)
    return yrs
...
def plotCoarsenedDataStats(dataDir, dlon, dlat, vmin, vmax):
  ...
  plt.pcolormesh(lonmap, latmap, cnt, transform=ccrs.PlateCarree(), vmin=vmin, vmax=vmax)
