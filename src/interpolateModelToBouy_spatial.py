...
def __elabFile(mdlF, mdlFPrev=None, mdlFNext=None, varNames=None):
    ...
    for i in range(nobs):
        ...
        tmstmpTidal, isSubset, idxMin, idxMax = get_subsest_list(
            tmstmpTidal_, min(tmmdl), max(tmmdl)
        )

        if not isSubset:
            print("Missing data station " + str(i))
            continue
        elif idxMin == idxMax:
            continue
        ...
        print(idxMin, idxMax, len(_resTidal[i][:]))
        print(res)
        print(lonn)
        print(latt)
    ...
    outFlName = mdlF.replace(".nc", "_" + "buoys")
    outFlPath = os.path.join(_destDir, outFlName)
    print(outFlName)
