...
def __elabFile(mdlF, mdlFPrev=None, mdlFNext=None, varNames=None):
  varNames = varNames if not varNames is None else ['longitude', 'latitude', 'hs', 'time']
  ...
  try:
    tmstrt = netCDF4.num2date(tmnc[0], tmnc.units, clndr, only_use_cftime_datetimes=False)
  except:
    print('something wrong in file ' + fpth)
    outFlPath = 'none'
    return outFlPath
  ...
  if not _startDate is None and not _endDate is None:
    tmend = netCDF4.num2date(tmnc[-1], tmnc.units, clndr, only_use_cftime_datetimes=False)
    if (_endDate < tmstrt) or (_startDate > tmend):
      print('  file out of date range. Skipping')
      ds.close()
      return outFlPath

  tmmdl = netCDF4.num2date(tmnc[:], tmnc.units, clndr, only_use_cftime_datetimes=False)
  ...
  try:
    ...
    if usePrev:
      ...
      try:
        ...
        tmmdl_ = netCDF4.num2date(tmnc_[-1], tmnc_.units, getNcCalendar(tmnc_), only_use_cftime_datetimes=False)
    ...
    if useNext:
      ...
      try:
        ds_ = netCDF4.Dataset(fpth_)
        tmnc_ = ds_.variables[timeVarName]
        tmmdl_ = netCDF4.num2date(tmnc_[0], tmnc_.units, getNcCalendar(tmnc_), only_use_cftime_datetimes=False)
  ...
  for tm, itm in zip(tmmdl, range(ntmmdl)):
    ...
    if _gridType == GRID_TYPE_UNSTRUCT:
      ...
      intpltr = RegularGridInterpolator(grdPoints, hsii, bounds_error=False, fill_value=np.nan)
      intp0[itm, :] = intpltr((latSat, lonSat))
...
def interpolateModelToCoarsenedSatData_WW3(crsSatDataDir, modelNcFileDir, destDir, boundaries=None, startDate=None, endDate=None, gridType=GRID_TYPE_UNSTRUCT, flpattern='(.*)\.nc$', overwriteExisting=True, nParWorker=12):
...
def interpolateModelToCoarsenedSatData_schismWWM(crsSatDataDir, modelNcFileDir, destDir, boundaries=None, startDate=None, endDate=None, flpattern='(.*)\.nc$', overwriteExisting=True, nParWorker=4):
  print("interpolating model data to sat")
  mdlfl0 = [f for f in os.listdir(modelNcFileDir) if re.match(flpattern, f)]
  #El problema esta en que guardo wwm_hot.nc por si acaso
  dts = []
  for fn in mdlfl0:
    if fn == "wwm_hot.nc":
      pass
    else:
      dts.append(int(re.match('schout_([0-9]*)\_compressed.nc', fn).groups(0)[0]))
#  dts = [int(re.match('schout_([0-9]*)\_compressed.nc', fn).groups(0)[0]) for fn in mdlfl0]
#  dts = [int(re.match('schout_([0-9]*)\.nc', fn).groups(0)[0]) for fn in mdlfl0]
  iii = np.argsort(dts)
...
def interpolateModelToCoarsenedSatData_WWMExperimental(crsSatDataDir, modelNcFileDir, destDir, boundaries=None, startDate=None, endDate=None, flpattern='(.*)\.nc$', overwriteExisting=True, nParWorker=4):
  print("interpolating model data to sat")
  mdlfl0 = [f for f in os.listdir(modelNcFileDir) if re.match(flpattern, f)]
...
if __name__ == '__main__':
 ...
 #destDir = '/media/lmentaschi/TOSHIBA EXT/analysis/WW3_UNST_VALIDATION_10yrs/hindcast_red_sea_unst_noobst/hsModelAndSatObs'
  ...
  interpolateModelToCoarsenedSatData(crsSatDataDir, modelNcFileDir, destDir, boundaries, startDate, endDate)
