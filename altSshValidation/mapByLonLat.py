import numpy as np
import xarray as xa
import geodiccaCoastLine


def mapByLonLat(times, lons, lats, msrs, mods, mapdx, mapdy, lonlims=[-180, 180], latlims=[-90, 90]):
  ...
  for tm, lon, lat, msr, mod in zip(times, lons, lats, msrs, mods):
    if mod > 20:
      continue
    if ( not (lonlims[0] <= lon) and (lon <= lonlims[1]) )\
         or ( not (latlims[0] <= lat) and (lat <= latlims[1]) ):
      continue
...
def createCoastlinePointsMask(maplons, maplats, resl='h', minDistFromCoast=20000):
  ...
  cstln = geodiccaCoastLine.coastLine(resl, llcrnr=[min(maplons), min(maplats)], urcrnr=[max(maplons), max(maplats)])
  ...
  for ix in range(len(maplons)):
    for iy in range(len(maplats)):
      ...
      if (np.sqrt(xdst**2. + ydst**2.) < minDistFromCoast):
        mask[iy, ix] = 0
      ipt += 1
      if ipt % 100 == 0:
...
def computeBiasNrmse(lons, lats, mapdata):
  ...
  for ix in range(len(lons)):
    for iy in range(len(lats)):
      ...
      _nrmse = (sum((mods - msrs)**2.)/sum(msrs**2.))**.5
      bias[iy, ix] = _bias
      nrmse[iy, ix] = _nrmse
  ...
  for ix in range(len(lons)):
    for iy in range(len(lats)):
      ...
      if len(mods) < nminobs:
        continue
      _obsSum = np.sum(msrs)
      _sqObsSum = np.sum(msrs**2.)
      _devSum = np.sum(mods - msrs)
      _sqDevSum = np.sum((mods - msrs)**2.)
