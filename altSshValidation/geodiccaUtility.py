...
earthCircle = 40000000.
earthRadius = earthCircle/(2 * pi)
deToRa = pi/180.
raToDe = 180./pi

seaMileToKm = 1.852
kmToSeaMile = 1./seaMileToKm


def mathDirToDegNPropDir(mathdir):
  return ((-mathdir + pi/2.) * raToDe) % 360.


def mathDirToDegNOrigDir(mathdir):
  return -mathDirToDegNPropDir(mathdir) % 360.


def greatCircleAngleRad(lon1, lat1, lon2, lat2):
  dlon = lon2 - lon1
  dlat = lat2 - lat1
  delta = abs(2 * asin( ( sin(dlat/2)**2. + cos(lat1)*cos(lat2)*sin(dlon/2)**2. )**0.5 ))
  return delta
...
def getNormalVersor(lon0, lat0, lon1, lat1):
  dy = lat1 - lat0
  avglat = (lat0 + lat1)/2. * deToRa
  assert avglat != 0, 'getNormalVersor: this function is affordable only far from the poles'
...
def normalIntersectSegment(normalPt, segpt0, segpt1):
  ...
  """  
  avgLat = (normalPt[1] + segpt0[1] + segpt1[1]) / 3. * deToRa
  assert abs(avgLat) < pi/2. * (89./90.), 'normalIntersectSegment: funcion invalid at the poles'
...
def rectanglesIntersect(minx0, maxx0, miny0, maxy0, minx1, maxx1, miny1, maxy1):
  segspoly = [(minx1, miny1), (minx1, maxy1), (maxx1, maxy1),\
                 (maxx1, miny1), (minx1, miny1)]

  oneAngleContained = pointInsidePolygon(minx0, miny0, segspoly) or\
                      pointInsidePolygon(minx0, maxy0, segspoly) or\
                      pointInsidePolygon(maxx0, maxy0, segspoly) or\
                      pointInsidePolygon(maxx0, miny0, segspoly)
  domainContainsSegs = (minx0 <= minx1) and (maxx0 >= maxx1)\
                    and (miny0 <= miny1) and (maxy0 >= maxy1)
  segsContainsDomain = (minx0 >= minx1) and (maxx0 <= maxx1)\
                    and (miny0 >= miny1) and (maxy0 <= maxy1)
  crossDomains1 = (minx0 <= minx1) and (maxx0 >= maxx1)\
              and (miny0 >= miny1) and (maxy0 <= maxy1)
  crossDomains2 = (minx0 >= minx1) and (maxx0 <= maxx1)\
              and (miny0 <= miny1) and (maxy0 >= maxy1)
  return oneAngleContained or domainContainsSegs or segsContainsDomain\
      or crossDomains1 or crossDomains2
...
def getIntersection(seg00, seg01, seg10, seg11):
  ...
  if x01 == x00:
    ...
    return
  if (min(x00, x01) <= x <= max(x00, x01)) and\
     (min(x10, x11) <= x <= max(x10, x11)) and\
     (min(y00, y01) <= y <= max(y00, y01)) and\
     (min(y10, y11) <= y <= max(y10, y11)):
    return x, y

def almostEqual(a, b, sigdigits = 7):
    return (a == b) or (int(round(a*10**sigdigits)) == int(round(b*10**sigdigits)))
...
def getPtInsidePolygon(poly):
  ...
  def addPtsOnSectionToPoly(minx, maxx, miny, maxy):
    ...
    for sg in segs:
      ...
      for bsg in blst:
        ...
        if intersectPt and (not intersectPt in result) and\
          not (\
            (almostEqual(intersectPt[0], sgp1[0]) and almostEqual(intersectPt[1], sgp1[1])) or\
            (almostEqual(intersectPt[0], sgp2[0]) and almostEqual(intersectPt[1], sgp2[1]))\
          ):
  ...
  def getPolySurfaceInSect(minx, maxx, miny, maxy):
    poly = addPtsOnSectionToPoly(minx, maxx, miny, maxy)
    p_ = [(p[0], p[1]) for p in poly if ((minx <= p[0] <= maxx) and (miny <= p[1] <= maxy))] 
    if not p_:
      if pointInsidePolygon(minx, miny, poly) or pointInsidePolygon(minx, maxy, poly)\
        or pointInsidePolygon(maxx, maxy, poly) or pointInsidePolygon(maxx, miny, poly):
...
def segmentLength(pt0, pt1):
  if not (almostEqual(pt0[0], pt1[0], 10) and almostEqual(pt0[1], pt1[1], 10)):
    return ((pt0[0] - pt1[0])**2. + (pt0[1] - pt1[1])**2.)**.5
  else:
    return 0
...
def angleBetweenPoints(ptv, pt1, pt2):
  ...
  if lv1 and lv2:
    angle = acos((lv1**2. + lv2**2. - l12**2.) / (2. * lv1 * lv2))
    return angle
  else:
    return float('NaN')
